...
exports.read = function(symbol, fd) {
  let strSymbol = symbol;
  let lenSymbol = strSymbol + '_len';
...
exports.write = function(id, fd) {
  let strSymbol, lenSymbol;
  if (id.type === 'Literal') {
    id.value = id.value.toString();
    strSymbol = util.format('__sys__def_%d', parseInt(Math.random()*100));
    lenSymbol = strSymbol + '_len';
    this.section.data.push(instructs.db(strSymbol, id.value));
    this.section.data.push(instructs.equ(lenSymbol, '$-'+strSymbol));
  } else {
    const variableInTable = this.variablesTable[id.name];
...
exports.unlink = function(path) {
  const symbol = util.format('_lamb_var_%d', parseInt(Math.random()*100));
