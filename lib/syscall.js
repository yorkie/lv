...
exports.write = function(str, fd) {
  ...
  if (str.type === 'Literal') {
    str.value = str.value.toString();
    strSymbol = util.format('_lamb_var_%d', parseInt(Math.random()*100));
    lenSymbol = strSymbol + '_len';
    this.section.data.push(instructs.db(strSymbol, str.value));
    this.section.data.push(instructs.equ(lenSymbol, '$-'+strSymbol));
  } else {

    console.log(this.variablesTable);
